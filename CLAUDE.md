...
- **Speech** ([`lib/deepgram.ts`](lib/deepgram.ts)): STT chọn model/ngôn ngữ theo `getSttConfig(lang)` (en→nova-3, zh/ja/ko→nova-2). TTS theo `ttsVoice` trong `lib/languages.ts`: en→Aura (`aura-asteria-en`), **ja→Aura-2 (`aura-2-uzume-ja`)**; zh/ko `ttsVoice=null` (Deepgram chưa hỗ trợ Trung/Hàn) → `/api/speech/tts` trả 204, client hiển thị câu hỏi dạng chữ. Mọi gate phát âm dựa trên `config.ttsVoice`.
